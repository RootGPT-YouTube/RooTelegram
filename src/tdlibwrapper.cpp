...
{
    const int eagerCustomEmojiSetPrefetchLimit = 4;
    int eagerCustomEmojiSetPrefetched = 0;
    ...
    while (stickerSetIterator.hasNext()) {
        ...
        const QString stickerType = stickerSet.value("sticker_type").toMap().value(_TYPE).toString();
        const bool isCustomEmojiSet = stickerType == QLatin1String("stickerTypeCustomEmoji");
        if (isCustomEmojiSet) {
            if (eagerCustomEmojiSetPrefetched >= eagerCustomEmojiSetPrefetchLimit) {
                continue;
            }
            eagerCustomEmojiSetPrefetched += 1;
        }
