...
namespace {
    ...
    QVariantList normalizedReactionsFromInteractionInfo(const QVariantMap &interactionInfo)
    {
        const QVariant reactionsValue = interactionInfo.value(REACTIONS);
        if (reactionsValue.type() == QVariant::List) {
            return reactionsValue.toList();
        }
        if (reactionsValue.type() == QVariant::Map) {
            // TDLib newer schema can wrap reactions inside messageReactions.reactions
            return reactionsValue.toMap().value(REACTIONS).toList();
        }
        return QVariantList();
    }
}
...
ChatModel::MessageData::MessageData(const QVariantMap &data, qlonglong msgid) :
    ...
    reactions(normalizedReactionsFromInteractionInfo(data.value(INTERACTION_INFO).toMap())),
...
uint ChatModel::MessageData::updateReactions(const QVariantMap &interactionInfo)
{
    ...
    reactions = normalizedReactionsFromInteractionInfo(interactionInfo);
...
void ChatModel::initialize(const QVariantMap &chatInformation)
{
    const qlonglong chatId = chatInformation.value(ID).toLongLong();
        // Reset completo dello stato per evitare contaminazione tra topic
    inReload = false;
    inIncrementalUpdate = false;
    searchModeActive = false;
    searchQuery.clear();
...
qlonglong ChatModel::getMessageThreadId() const
{
    return messageThreadId;
}

void ChatModel::setMessageThreadId(qlonglong threadId)
{
...
qlonglong ChatModel::getTopicLastMessageId() const
{
    return topicLastMessageId;
}

void ChatModel::setTopicLastMessageId(qlonglong lastMsgId)
{
        ...
        if (this->isMostRecentMessageLoaded() || this->inIncrementalUpdate) {
            ...
            if (!messagesToBeAdded.isEmpty() && (messagesToBeAdded.size() + messages.size()) < 10 && !inReload) {
                ...
                if (this->searchModeActive) {
                    ...
                    // Non caricare ulteriormente per OFFTOPIC in scroll su
                } else {
                    this->tdLibWrapper->getChatHistory(chatId, messagesToBeAdded.first()->messageId, 0);
                }
            } else {
