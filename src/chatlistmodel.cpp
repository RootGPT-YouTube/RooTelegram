...
void ChatListModel::handleChatPendingJoinRequestsUpdated(qlonglong chatId, const QVariantMap &pendingJoinRequests)
{
    if (chatIndexMap.contains(chatId)) {
        LOG("Updating pending join requests for" << chatId << "count:" << pendingJoinRequests.value(TOTAL_COUNT).toInt());
        ...
        if (chat) {
            LOG("Updating pending join requests for hidden chat" << chatId << "count:" << pendingJoinRequests.value(TOTAL_COUNT).toInt());
            chat->chatData.insert(PENDING_JOIN_REQUESTS, pendingJoinRequests);
            return;
        }
        chat = folderFilteredChats.value(chatId);
        if (chat) {
            LOG("Updating pending join requests for folder filtered chat" << chatId << "count:" << pendingJoinRequests.value(TOTAL_COUNT).toInt());
            chat->chatData.insert(PENDING_JOIN_REQUESTS, pendingJoinRequests);
        }
    }
}
...
QString ChatListModel::ChatData::senderMessageText() const
{
    ...
    return RooTelegramUtils::getMessageShortText(tdLibWrapper, lastMessage(CONTENT).toMap(), isChannel(), myUserId, lastMessage(SENDER_ID).toMap() );
}
...
ChatListModel::ChatListModel(TDLibWrapper *tdLibWrapper, AppSettings *appSettings) : showHiddenChats(false), activeFolderId(0)
{
...
ChatListModel::~ChatListModel()
{
    ...
    qDeleteAll(folderFilteredChats.values());
}
...
void ChatListModel::handleChatDiscovered(const QString &, const QVariantMap &chatToBeAdded)
{
    ...
    const QString initialOrder = chatToBeAdded.value(ORDER).toString();
    if (!initialOrder.isEmpty()) {
        folderOrders[chat->chatId][0] = initialOrder;
    }

    // Leggi il campo "positions" per pre-popolare le posizioni nelle cartelle.
    // Questo è CRITICO: senza questo, i folderOrders per le cartelle vengono popolati
    // solo da eventi updateChatPosition asincroni che arrivano DOPO che l'utente
    // ha già selezionato una cartella, causando una lista vuota/sbagliata.
    const QVariantList positions = chatToBeAdded.value("positions").toList();
    for (const QVariant &posVar : positions) {
        const QVariantMap posMap = posVar.toMap();
        const QVariantMap listMap = posMap.value("list").toMap();
        const QString listType = listMap.value("@type").toString();
        if (listType == "chatListFolder") {
            const int folderId = listMap.value("chat_folder_id").toInt();
            const QString folderOrder = posMap.value("order").toString();
            if (folderId > 0 && !folderOrder.isEmpty()) {
                folderOrders[chat->chatId][folderId] = folderOrder;
                LOG("Pre-populated folder order for chat" << chat->chatId << "folder" << folderId << "order" << folderOrder);
            }
        }
    }
...
void ChatListModel::handleChatOrderUpdated(const QString &id, const QString &order)
{
    ...
    if (ok) {
        if (chatIndexMap.contains(chatId)) {
            ...
            // Aggiorna SEMPRE l'ordine principale (anche quando siamo in una cartella)
            // altrimenti quando torniamo a "Tutte" usiamo un ordine stale
            if (!order.isEmpty()) {
                folderOrders[chatId][0] = order;
            }
            int chatIndex = chatIndexMap.value(chatId);
            ...
            if (chat) {
                ...
                // Chat filtrata per cartella: aggiorna solo folderOrders[0], non la posizione visiva
                if (!order.isEmpty()) {
                    folderOrders[chatId][0] = order;
                }
            }
...
void ChatListModel::handleChatFolderPositionUpdated(const QString &chatId, int folderId, const QString &order, bool isPinned)
{
    bool ok;
    const qlonglong id = chatId.toLongLong(&ok);
    if (!ok) return;

    // Salva l'ordine della cartella per questa chat
    folderOrders[id][folderId] = order;

    if (activeFolderId == folderId) {
        // Questa chat appartiene alla cartella attiva con un ordine valido?
        const bool hasValidOrder = !order.isEmpty() && order != QStringLiteral("0");

        if (hasValidOrder) {
            // Se la chat era filtrata fuori, ora va rimessa in lista
            if (folderFilteredChats.contains(id)) {
                ChatData *chat = folderFilteredChats.take(id);
                chat->setOrder(order);
                if (!chat->isHidden() || showHiddenChats) {
                    addVisibleChat(chat);
                }
            } else if (chatIndexMap.contains(id)) {
                // È già in lista: aggiorna ordine e posizione
                LOG("Updating folder position for chat" << id << "folder" << folderId << "order" << order);
                int chatIndex = chatIndexMap.value(id);
                if (chatList.at(chatIndex)->setOrder(order)) {
                    updateChatOrder(chatIndex);
                }
                if (chatList.at(chatIndexMap.value(id))->chatData.value("is_pinned").toBool() != isPinned) {
                    ChatData *chat = chatList.at(chatIndexMap.value(id));
                    chat->chatData.insert("is_pinned", isPinned);
                    const int idx = chatIndexMap.value(id);
                    emit dataChanged(index(idx), index(idx));
                }
            }
        } else {
            // Ordine zero/vuoto = chat rimossa dalla cartella: filtrala
            if (chatIndexMap.contains(id)) {
                int chatIndex = chatIndexMap.value(id);
                ChatData *chat = chatList.at(chatIndex);
                ...
                chatIndexMap.remove(id);
                folderFilteredChats.insert(id, chat);
                endRemoveRows();
                emit countChanged();
            }
        }
    } else if (activeFolderId == 0 && folderId == 0) {
        // Aggiornamento posizione nella lista principale
        if (chatIndexMap.contains(id)) {
            int chatIndex = chatIndexMap.value(id);
            LOG("Updating main position for chat" << id << "order" << order);
            if (chatList.at(chatIndex)->setOrder(order)) {
                updateChatOrder(chatIndex);
            }
        }
...
void ChatListModel::setActiveFolder(int folderId)
{
    if (activeFolderId == folderId) return;
    activeFolderId = folderId;

    // ── Passo 1: ripristina sempre le chat filtrate dalla cartella precedente ──
    // Rimettiamo in chatList tutte le chat in folderFilteredChats
    if (!folderFilteredChats.isEmpty()) {
        const QList<ChatData*> toRestore = folderFilteredChats.values();
        folderFilteredChats.clear();
        for (ChatData *chat : toRestore) {
            // Ripristina l'ordine principale (folder 0)
            if (folderOrders.contains(chat->chatId) && folderOrders[chat->chatId].contains(0)) {
                chat->setOrder(folderOrders[chat->chatId][0]);
            }
            // Aggiunge di nuovo alla lista visibile (se non è già hidden per altri motivi)
            if (!chat->isHidden() || showHiddenChats) {
                addVisibleChat(chat);
            } else {
                hiddenChats.insert(chat->chatId, chat);
    ...
    if (folderId == 0) {
        // ── "Tutte": già ripristinato sopra, riordina e fine ──
        LOG("Switching to main chat list");
        // Aggiorna gli ordini principali e riordina
        beginResetModel();
        ...
        std::sort(chatList.begin(), chatList.end(), [](const ChatData *a, const ChatData *b) {
    ...
    // ── Passo 2: switch a cartella specifica ─────────────────────────
    // Strategia robusta per primo accesso e accessi successivi:
    // 1. Sposta TUTTE le chat in folderFilteredChats (lista pulita)
    // 2. Re-aggiungi subito quelle con posizione già nota per questa cartella
    // 3. Le restanti arrivano via handleChatFolderPositionUpdated man mano
    //    che TDLib invia gli updateChatPosition — funziona al primo E successivi accessi
    LOG("Switching to folder" << folderId);

    beginResetModel();

    // Sposta tutto in folderFilteredChats
    for (ChatData *chat : chatList) {
        folderFilteredChats.insert(chat->chatId, chat);
    }
    chatList.clear();
    chatIndexMap.clear();

    // Re-aggiungi immediatamente le chat con posizione nota per questa cartella
    QList<qlonglong> toReAdd;
    for (auto it = folderFilteredChats.begin(); it != folderFilteredChats.end(); ++it) {
        const qlonglong cid = it.key();
        if (folderOrders.contains(cid)
                && folderOrders[cid].contains(folderId)
                && !folderOrders[cid][folderId].isEmpty()
                && folderOrders[cid][folderId] != QStringLiteral("0")) {
            toReAdd.append(cid);
        }
    }
    for (const qlonglong cid : toReAdd) {
        ChatData *chat = folderFilteredChats.take(cid);
        chat->setOrder(folderOrders[cid][folderId]);
        chatList.append(chat);
    }

    // Ordina le chat re-aggiunte
    std::sort(chatList.begin(), chatList.end(), [](const ChatData *a, const ChatData *b) {
        return a->compareTo(b) < 0;
    });
    for (int i = 0; i < chatList.size(); i++) {
    ...
    endResetModel();
    emit countChanged();
    LOG("Folder" << folderId << ": showing" << chatList.size()
        << "known chats, waiting for" << folderFilteredChats.size() << "position updates");
}
